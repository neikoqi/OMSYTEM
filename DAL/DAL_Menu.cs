...
namespace DAL
{
    ...
    public static bool del(int id)///根据id来删除
        {   
            bool result = false;
            string strcon = "delete  from [dbo].[Menu] where id =" + id + "";
   ...
   /// <summary>
    ...
    /// <returns></returns>
        ...
        bool result= false;
            string strcon = "insert into [dbo].[Menu](title,state) values('"+menu.Title+"','"+menu.State+"')";
    ...
    /// 例表查询
    /// </summary>
    public static List<MODELS.MOD_Menu> list() /*查询转化table为list*/
    {
        string strcon = "select * from [dbo].[Menu]";
        DataTable dataTable = UTILITY.MsSqlHelper.YFMsSqlHelper.Query(strcon).Tables[0];/* 查询table参数为0*/
        return daTolist(dataTable);

    }

    public static List<MODELS.MOD_Menu>daTolist(DataTable dataTable)///转化类型
    {
        List<MOD_Menu> list = new List<MOD_Menu>();
        ...
        {
            string strcon = "select * from [dbo].[Menu] where id="+id+"";/*得到某一条记录*/
            DataTable dataTable = UTILITY.MsSqlHelper.YFMsSqlHelper.Query(strcon).Tables[0];/* 查询table参数为0*/
